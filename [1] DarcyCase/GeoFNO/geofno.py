...
model = FNO2d(
    ...
    in_channels=2,
    out_channels=1,
    is_mesh=False,   # ⚠️ 非规则点，一定要 False
    s1=32,
    s2=32
).to(device)
...
# 第一组
x1 = torch.rand(B, N1, 2, device=device)
u1 = x1.clone()

# 第二组
x2 = torch.rand(B, N2, 2, device=device)
u2 = x2.clone()

# ----------------- Forward with IPHI -----------------
with torch.no_grad():
    y1 = model(u=u1, x_in=x1, x_out=x1, iphi=iphi)
    y2 = model(u=u2, x_in=x2, x_out=x2, iphi=iphi)

print("Group 1 output shape:", y1.shape)
print("Group 2 output shape:", y2.shape)
